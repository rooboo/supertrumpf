...
export default class Game extends React.Component<GameProps, GameState> {
    ...
    compare(propertyName: string) {
        ...
        let playerAnimal = update(this.state.player, {$splice: [[0, 1]]}); // remove one item from array starting from position one
        const firstComputer = this.state.computer[0];
        let computerAnimal = update(this.state.computer, {$splice: [[0, 1]]}); // remove one item from array starting from position one
        ...
        if (playerPropValue > computerPropValue) {
            playersTurn = true;
            playerAnimal = update(playerAnimal, {$push: [firstPlayer, firstComputer]}); // add computer card to player cards
            ...
            playersTurn = false;
            computerAnimal = update(computerAnimal, {$push: [firstPlayer, firstComputer]}); // add player card to computer cards
